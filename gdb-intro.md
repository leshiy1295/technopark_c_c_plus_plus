...
Пока же, если `__builtin_FUNCTION` не подошел, придется использовать макросы
(`__FILE__` и `__LINE__` - тоже расширения компилятора, но более
распространенные). Аналогичный по функциональности фрагмент кода, использующий
...
сокращать, как правило сокращенное имя команды - это первая буква в названии
...
используем команду `continue` (сокращенно `c`):
...
режим не до конца отлажен (отлаживать отладчик видимо не так-то просто) и в нем
